...
def class_generate_bao(cosmo,
                        rootName = 'class_generate_bao_two',
                        cmbNoise = None,
                        noiseLevel = 1.,
                        beamSizeArcmin = 1.,
                        deflectionNoise = None,
    ...
    if 'xi_idr' in list(cosmo.keys()):
            cosmoclass['xi_idr'] = cosmo['xi_idr']
    if 'N_idr' in list(cosmo.keys()):
        cosmoclass['N_idr'] = cosmo['N_idr']
        cosmoclass['idr_nature'] = 'fluid' # Could be free-streaming or fluid, this is just convenient for now
    if 'stat_f_idr' in list(cosmo.keys()):
        cosmoclass['stat_f_idr'] = cosmo['stat_f_idr']
    if 'a_idm_dr' in list(cosmo.keys()):
        cosmoclass['a_idm_dr'] = cosmo['a_idm_dr']
        cosmoclass['idr_nature'] = 'fluid' # Could be free-streaming or fluid, this is just convenient for now
    if 'Gamma_0_nadm' in list(cosmo.keys()):
        cosmoclass['Gamma_0_nadm'] = cosmo['Gamma_0_nadm']
    if 'nindex_idm_dr' in list(cosmo.keys()):
        cosmoclass['nindex_idm_dr'] = cosmo['nindex_idm_dr']
    if 'b_idr' in list(cosmo.keys()):
        cosmoclass['b_idr'] = cosmo['b_idr']
    if 'alpha_idm_dr' in list(cosmo.keys()):
        cosmoclass['alpha_idm_dr'] = cosmo['alpha_idm_dr']
    if 'beta_idr' in list(cosmo.keys()):
        cosmoclass['beta_idr'] = cosmo['beta_idr']
...
########################################################
    ...
    ########################################################
        ...
        cspec['cl_unlensed'] = None # To be filled with unlensed sCMB spectra
        cspec['cl_lensed'] = None # To be filled with lensed CMB spectra
        cspec['cl_delensed'] = None # To be filled with delensed CMB spectra
        cspec['nl_lensing'] = None # To be filled with lensing reconstruction noise
        cspec['nl_cmb'] = None # To be filled with CMB spectra noise
...
def getBAOParams(cosmo, redshifts, extraParams = dict()):    
    # returns rs/dV for desired redshifts
    class_generate_bao(cosmo = cosmo, extraParams = extraParams)
    ...
    data = []
    with open("CLASS_delens/output/class_generate_bao_two_background.dat", 'r') as f:
        d = f.readlines()
        for i in d:
            k = i.rstrip().split("       ")
            data.append([float(i) if is_float(i) else i for i in k]) 
    
    a_file = open("CLASS_delens/output/class_generate_thermodynamics.txt")
    for position, line in enumerate(a_file):
        if position == 20:
            rs = float(line)

    numComments = 8
    x = [None for j in range(numComments,len(data)-1)]
    y = [None for j in range(numComments,len(data)-1)]
    
    for i in range(0,len(data)-numComments-1):
        
        c = 1.
        dA = (data[i+numComments][6])
        H = (data[i+numComments][4])
        z = (data[i+numComments][1])
        print(dA)
        print(H)
        print(z)
        rs_dV = rs/(((c)*z*((1+z) ** 2.) * (dA ** 2.) * (H ** -1.)) ** (1./3))
    ...
    return np.interp(redshifts,x,y)